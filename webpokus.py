...
def display_five_player_segments():
    ...
    if selected_team:
        ...
        cursor.execute("""
            SELECT g.game_id, g.date, g.location, 
        ...
        # Create match options for dropdown with scores displayed next to team names
        match_options = []
        for match in matches:
            game_id, date, location, home_team, away_team, home_score, away_score = match
            match_display = f"{date} - {home_team} {home_score}-{away_score} {away_team} ({location})"
            match_options.append((match_display, game_id))
        
        # Display match selection dropdown
        selected_match_display = st.selectbox(
            "Select a match:",
            options=[m[0] for m in match_options],
            format_func=lambda x: x
        )
        
        # Find the game_id for the selected match
        selected_game_id = None
        for match_display, game_id in match_options:
            if match_display == selected_match_display:
