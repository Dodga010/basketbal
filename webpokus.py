...
def display_five_player_segments():
    ...
    # Step 1: Get teams and create team dropdown
    teams = fetch_teams()
    team_options = [team[1] for team in teams]  # Assuming team[1] is the team name
    selected_team = st.selectbox("Select Team", options=team_options)
    
    if selected_team:
        # Step 2: Get matches for the selected team
        team_matches = fetch_team_matches(selected_team)
        
        if team_matches and len(team_matches) > 0:
            # Create a dictionary to map display text to game_id
            match_id_to_index = {}
            match_options = []
            
            # Format match options to include scores
            for match in team_matches:
                game_id = match[0]
                
                # Fetch final scores for this game
                scores = fetch_final_scores(game_id)
                if scores:
                    home_team = scores[0]
                    away_team = scores[1]
                    home_score = scores[2]
                    ...
                    # Format: "Home Team 75 - 82 Away Team"
                    display_text = f"{home_team} {home_score} - {away_score} {away_team}"
                    match_options.append(display_text)
                    match_id_to_index[display_text] = game_id
            
            # Step 3: Create match dropdown with formatted options
            if match_options:
                selected_match = st.selectbox("Select Match", options=match_options)
                
                if selected_match:
                    # Get the game_id for the selected match
                    game_id = match_id_to_index[selected_match]
                    
                    # Analyze and display five player segments for the selected game
                    with st.spinner("Analyzing five player segments..."):
                        lineup_data = analyze_five_player_segments(game_id)
                    
                    if lineup_data is not None and not lineup_data.empty:
                        st.write("Five Player Segments Analysis:")
                        st.dataframe(lineup_data)
                        
                        # You can add additional visualizations here based on your preferences
                        if st.checkbox("Show Lineup Performance Chart"):
                            fig = px.bar(
                                lineup_data, 
                                x="Lineup", 
                                y="Net Rating",  # Assuming this column exists
                                title=f"Lineup Performance for {selected_match}",
                ...
                st.warning("No matches found with score data for the selected team.")
        else:
