...
def display_five_player_segments():
    """Display five player segments analysis with team selection first"""
    ...
    if selected_team:
        ...
        team_matches = fetch_team_games(selected_team)
        
        if not team_matches:
            st.warning(f"No matches found for {selected_team}.")
            return
            
        # Create options for the match dropdown with scores included
        match_options = {}
        for game_id, opponent in team_matches:
            ...
            WHERE t1.game_id = ? AND t1.name = ?
            """
            cursor = conn.cursor()
            cursor.execute(score_query, (game_id, selected_team))
            result = cursor.fetchone()
            conn.close()
            
            if result:
                team_score, opponent_score = result
                match_display = f"{selected_team} vs {opponent} ({team_score}-{opponent_score})"
                match_options[match_display] = game_id
        
        # Match selection dropdown with scores
